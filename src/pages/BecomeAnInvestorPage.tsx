...
export const BecomeAnInvestorPage = () => {
  ...
  const shimmerStyles = `
    ...
    @keyframes investorSparkle {
      ...
      100% {
        opacity: 0;
        transform: translate(-50%, -140%) scale(0.5);
      }
    }

    @keyframes investorPulse {
      0% {
  ...
  `;
  const sparklePositions = [0.18, 0.42, 0.66, 0.85];
  const sparkleLeft = (ratio: number, percentage: number) => Math.min(ratio * percentage, percentage);
  const renderSparkleStar = ({
    size,
    prefix,
    percentage,
    delayStep,
    ...
    sparklePositions.map((ratio, index) => (
      <svg
        key={`${prefix}-sparkle-${index}`}
        viewBox="0 0 24 24"
        className="absolute -translate-x-1/2 -translate-y-1/2 text-[#fafecc] drop-shadow-[0_0_12px_rgba(208,238,138,0.9)]"
        style={{
          width: size,
          height: size,
          left: `${sparkleLeft(ratio, percentage)}%`,
          top: '50%',
          animation: `investorSparkle 3s ease-in-out infinite`,
          animationDelay: `${index * delayStep}s`,
        }}
      >
        <polygon
          fill="currentColor"
          points="12,2 14.8,8.5 22,9.3 16.5,13.9 18.1,21 12,17.4 5.9,21 7.5,13.9 2,9.3 9.2,8.5"
        />
      </svg>
    ));
  const animatedProgress = hasProgressAnimated ? progressPercentage : 0;

  useEffect(() => {
    if (!progressSectionRef.current) return;
    if (typeof window === 'undefined' || !(window as any).IntersectionObserver) {
      setHasProgressAnimated(true);
      return;
    }
  ...
  const pitchDeckSlides = [
    {
      ...
      title: 'The Opportunity in Front of Us',
      description: 'Aasta is built to turn wasted food into accessible meals. We connect restaurants with people who need affordable food, creating a system that reduces waste and supports students, workers, and everyday consumers.',
    },
    {
      id: 2,
      image: '/pitch-deck/slide-2.jpg',
      title: 'Built With Users From Day One',
      description: 'We didn’t wait for launch to understand the market. We went into hostels, cafés, and streets to test the idea in real conditions. These conversations shaped the product and confirmed that people want a reliable way to access quality surplus food.',
    },
    {
      id: 3,
      image: '/pitch-deck/slide-3.jpg',
      title: 'A ₹30,000 Cr Market Hiding in Plain Sight',
      description: 'India’s HoReCa industry wastes over ₹1.5 lakh crore worth of food annually. At least ₹30,000 crore of that is still consumable and monetizable. Our initial focus is on surplus from 20,000+ restaurants in Bangalore, unlocking a ₹200 crore/year opportunity. This is just the start. We’re building for 10 major cities next.',
    },
    {
      id: 4,
      image: '/pitch-deck/slide-4.jpg',
      title: 'A Vast Pool of Edible Surplus Left Unused',
      description: 'Most food discarded by the industry is still safe to eat but has no channel to reach buyers before it’s dumped. This is lost revenue for vendors and lost access for consumers. Aasta converts this edible surplus into a monetizable stream, creating value where the system currently burns it.',
    },
    {
      id: 5,
      image: '/pitch-deck/slide-5.jpg',
      title: 'Tapping Into a High-Volume, Underserved Market',
      description: 'Millions of workers, gig riders, and low-income consumers avoid food delivery apps because prices and fees are too high. They rely on cheap local options instead. Aasta gives them access to quality surplus food at prices they can actually afford, unlocking a new demand segment that the major platforms do not serve today.',
    },
    {
      id: 6,
      image: '/pitch-deck/slide-6.jpg',
      title: 'Multiple Revenue Streams Built for Scale',
      description: 'Our model earns from both sides of the marketplace. Restaurants pay commissions and placement fees. Customers pay a small platform charge. Brands can sponsor products, and new formats like surprise boxes and subscriptions unlock additional upside. This mix keeps margins healthy and reduces dependency on a single revenue line.',
    },
    {
      id: 7,
      image: '/pitch-deck/slide-7.jpg',
      title: 'Strong Early Traction With Clear Signals',
      description: 'In a short period, we delivered hundreds of orders, saved significant food from waste, and built a repeat customer base with zero complaints. User growth is steady, vendors are returning, and restaurants are already seeing measurable savings. This early traction shows the model works in the real world and is ready to scale.',
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="text-center">
            ...
            <p className="text-base sm:text-xl md:text-2xl text-white/90 max-w-2xl sm:max-w-3xl mx-auto font-medium mb-8 px-2">
              Donate in Aasta — Where Every Meal Saved, Feeds Someone
            </p>

            <div className="flex justify-center mb-16 sm:mb-24 md:mb-16">
              <button
                type="button"
                  ...
                  <div className="relative h-4 bg-primary-600/60 rounded-full border border-primary/30 overflow-hidden shadow-[0_0_28px_rgba(208,238,138,0.25)]">
                    ...
                    >
                      <div
                        ...
                        style={{ animation: 'investorShimmer 2.4s linear infinite' }}
                      ></div>
                    ...
                    <div
                      className="absolute inset-y-0 right-0 bg-primary-600 rounded-full"
                      style={{ width: `${Math.max(100 - animatedProgress, 0)}%` }}
                    ></div>
                  </div>
                  {animatedProgress > 0 && (
                    <>
                      {/* <div
                        className="pointer-events-none absolute top-1/2 w-6 h-6 rounded-full bg-[#fafecc] shadow-[0_0_25px_rgba(208,238,138,0.7)]"
                        style={{
                          left: `calc(${animatedProgress}% - 12px)`,
                          animation: 'investorPulse 1.8s ease-in-out infinite',
                        }}
                      ></div> */}
                      <div
                        className="pointer-events-none absolute top-0 bottom-0 w-12 bg-gradient-to-r from-transparent via-primary/40 to-transparent"
                        style={{
                          left: `calc(${animatedProgress}% - 20px)`,
                          animation: 'investorReach 1.6s ease-in-out infinite',
                        }}
                      ></div>
                    </>
                  )}
                ...
                <div className="relative">
                  <div className="relative h-8 top-12 mb-12 sm:mb-16">
                    ...
                    <div className="absolute inset-0">
                      ...
                      >
                        <div
                          className="absolute inset-0 bg-gradient-to-r from-transparent via-white/90 to-transparent opacity-80"
                          style={{ animation: 'investorShimmer 2.6s linear infinite' }}
                        ></div>
                        <div className="pointer-events-none absolute inset-0">
                          {renderSparkleStar({
                            size: 20,
                    ...
                    <div
                      ...
                      style={{
                        width: `${Math.max(100 - animatedProgress, 0)}%`,
                      }}
                    ></div>
                    </div>
                    {animatedProgress > 0 && (
                      <>
                        {/* <div
                          className="pointer-events-none absolute top-1/2 w-8 h-8 rounded-full bg-[#fafecc] shadow-[0_0_28px_rgba(208,238,138,0.9)]"
                          style={{
                            left: `calc(${animatedProgress}% - 16px)`,
                            animation: 'investorPulse 1.8s ease-in-out infinite',
                          }}
                        ></div> */}
                        <div
      ...
      {/* Benefits Section */}
      <section className="py-16 sm:py-24 mb-40 bg-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="font-dela text-4xl sm:text-5xl md:text-7xl text-primary mb-24 uppercase">
              Usage of your investment
            </h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
            <div className="bg-card-bg rounded-[2rem] p-6 sm:p-8 border-2 border-primary/30 flex flex-col justify-between">
              <div>
                <h3 className="font-dela text-xl sm:text-2xl text-primary mb-2 text-center">
                  Marketing
                </h3>
                <p className="text-white/80 text-sm sm:text-base text-center">
                  Building awareness, capturing early users.
                </p>
              </div>
            </div>
            <div className="bg-card-bg rounded-[2rem] p-6 sm:p-8 border-2 border-primary/30 flex flex-col justify-between">
              <div>
                ...
                <p className="text-white/80 text-sm sm:text-base text-center">
                  Building AASTA app, scalable, reliable, and intuitive.
                </p>
              </div>
            </div>
            <div className="bg-card-bg rounded-[2rem] p-6 sm:p-8 border-2 border-primary/30 flex flex-col justify-between">
              <div>
                ...
                <p className="text-white/80 text-sm sm:text-base text-center">
                  Salaries for team: capable sales, operations.
                </p>
              </div>
            </div>
            <div className="bg-card-bg rounded-[2rem] p-6 sm:p-8 border-2 border-primary/30 flex flex-col justify-between">
              <div>
                ...
                <p className="text-white/80 text-sm sm:text-base text-center">
                  Positioning, expansion, reach franchises, packaging.
                </p>
              </div>
            </div>
          </div>
